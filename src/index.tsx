...
export default function HTML2JSX({ innerHTML, convert, enableScript }: HTML2JSXProps) {
    let str = innerHTML
    ...
    for (; ;) {
        ...
        if (matchTag1 || matchTag2) {
            ...
            if (convert) {
                JSXList.push(createElement(convert, { HTMLProps, eventProps, tagName: "", originalElement: originalTextElement }, str.slice(0, index)))
            } else {
            ...
            if (!startTag.endsWith("/>")) {
                ...
                if (endIndex >= index + startTag.length) {
                    if (tagName.toLowerCase() !== "script" || enableScript) {
                        const originalTagElement = createElement(tagName, HTMLProps, <HTML2JSX innerHTML={str.slice(index + startTag.length, endIndex)} />)
                        if (convert) {
                            JSXList.push(createElement(convert, { HTMLProps, eventProps, tagName, originalElement: originalTagElement }, <HTML2JSX innerHTML={str.slice(index + startTag.length, endIndex)} />))
                        } else {
            ...
            if (tagName.toLowerCase() !== "script" || enableScript) {
                ...
                if (convert) {
                    JSXList.push(createElement(convert, { HTMLProps, eventProps, tagName: tagName, originalElement: originalTagElement }))
                } else {
                    JSXList.push(originalTagElement)
                }
            }
    ...
    return (
        createElement(Fragment, {}, ...JSXList)
    )
}